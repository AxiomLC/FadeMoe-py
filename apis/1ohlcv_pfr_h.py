# 12 Dec 25, refactored for COPY inserts
import asyncio
import time
import traceback
...
SCRIPT_NAME = "1ohlcv_pfr_h.py"
SCRIPT_DEF = "OHLCV & PFR Backfill"
HEARTBEAT_INTERVAL_SECONDS = 15
FINAL_LOOP_MINUTES = 4
BATCH_INSERT_SIZE = 10000 #4000
TOTAL_CONCURRENCY = 9 #8
API_TIMEOUT = 5
INITIAL_BACKOFF_DELAY = 0.7 #.7
MAX_RETRIES = 4 #4
...
def process_data(raw_data, base_symbol, data_type):
    try:
        if data_type == 'ohlcv':
            return [{"ts": int(d[0]), "symbol": base_symbol, "o": float(d[1]), "h": float(d[2]),
                     "l": float(d[3]), "c": float(d[4]), "v": float(d[7])} for d in raw_data]
        if data_type == 'pfr':
            return [{"ts": int(d[0]), "symbol": base_symbol, "pfr": float(d[4])} for d in raw_data]
    except (ValueError, IndexError):
        # Errors will be logged by the caller if processing fails
        pass
    return []
...
def create_mt_symbol_data(all_ohlcv_data):
    ...
    for ts, records in mt_data_map.items():
        if len(records) == len(MT_SYMBOLS):
            mt_records.append({"ts": ts, "symbol": "MT",
                               "o": sum(r["o"] for r in records) / len(records),
                               "h": sum(r["h"] for r in records) / len(records),
                               "l": sum(r["l"] for r in records) / len(records),
                               "c": sum(r["c"] for r in records) / len(records),
                               "v": sum(r["v"] for r in records)})
    return mt_records
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if response.status_code == 200:
                    ...
                    await log_error(db_manager, SCRIPT_NAME, "API Error",
                                    f"{proxy_name} | {symbol} | {api_name} | HTTP {response.status_code} | Retrying...")
                    await asyncio.sleep(delay)
                    delay *= 2
            except Exception as e:
                error_msg = e.__class__.__name__.replace("Error", "")
                await log_error(db_manager, SCRIPT_NAME, "Client Error",
                                f"{proxy_name} | {symbol} | {api_name} | {error_msg} | Retrying...")
...
async def fetch_and_process_symbol(session, base_symbol, semaphore, db_manager, state, proxy_map, is_final_loop):
    async with semaphore:
        ...
        start_time_ms = int(
            (datetime.now() - timedelta(days=DB_RETENTION_DAYS if not is_final_loop else 0,
                                        minutes=FINAL_LOOP_MINUTES if is_final_loop else 0)).timestamp() * 1000)

        results = await asyncio.gather(
            fetch_data(session, binance_symbol, BINANCE_API_CONFIG["ohlcv"], state, start_time_ms, proxy_map, db_manager,
                       is_final_loop, "OHLCV"),
            fetch_data(session, binance_symbol, BINANCE_API_CONFIG["pfr"], state, start_time_ms, proxy_map, db_manager,
                       is_final_loop, "PFR"),
        ...
        # Combine batches into one insert with merge_data_to_stage
        total_inserted = 0
        if processed_ohlcv or processed_pfr:
            try:
                total_inserted = await db_manager.merge_data_to_stage(processed_ohlcv, processed_pfr)
                state.total_records_inserted += total_inserted
            except Exception as e:
                await log_error(db_manager, SCRIPT_NAME, "Insert Error", f"{base_symbol} :: {e}", details=traceback.format_exc())

        return processed_ohlcv

#======= Run cycle for all symbols =======
async def run_fetch_cycle(session, db_manager, state, proxy_map, is_final_loop=False):
    ...
    tasks = [fetch_and_process_symbol(session, sym, semaphore, db_manager, state, proxy_map, is_final_loop) for sym in
             BASE_SYMBOLS]
    ...
    for future in asyncio.as_completed(tasks):
        ...
        if ohlcv_list:
            all_ohlcv_for_mt.extend(ohlcv_list)

    mt_symbol_data = create_mt_symbol_data(all_ohlcv_for_mt)
    if mt_symbol_data:
        total_inserted = 0
        for i in range(0, len(mt_symbol_data), BATCH_INSERT_SIZE):
            batch = mt_symbol_data[i:i + BATCH_INSERT_SIZE]
            try:
                count = await db_manager.merge_data_to_stage(batch)
                if count > 0:
                    total_inserted += count
            except Exception as e:
                await log_error(db_manager, SCRIPT_NAME, "Insert Error", f"MT Batch :: {e}", details=traceback.format_exc())
...
async def main():
    ...
    try:
        async with AsyncSession() as session:
            ...
            await log_status(db_manager, SCRIPT_NAME, "Running", "OHLCV Backfill Complete - Starting Final Loop (to Now)")
        ...
        completion_message = f"⏱️ {SCRIPT_DEF} Complete - Inserted {state.total_records_inserted:,} records in {duration:.2f}s."
