...
try:
    ...
    from back.api_utils import format_symbol
...
# ============================================================================
#  2. SELF-CONTAINED LOGGING & STATE
# ============================================================================
LOG_CYAN = Fore.CYAN
...
class ScriptState:
    def __init__(self):
        # REVISION: Track fetched records per proxy
...
def process_data(raw_data, base_symbol, data_type):
    try:
        ...
        print(LOG_RED + f"Processing Error | {base_symbol} | {data_type} | {e}")
    return []
...
# ============================================================================
#  4. CORE API & DATABASE LOGIC (DATA MIXING BUG FIXED)
# ============================================================================
async def fetch_data(session, symbol, config, state, start_time_ms, proxy_map, is_final_loop=False, api_name="UNKNOWN"):
    all_data = []
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(LOG_RED + f"{proxy_name} | {symbol} | {api_name} | HTTP {response.status_code} | Retrying...")
                    await asyncio.sleep(delay)
                    delay *= 2
            except Exception as e:
                error_msg = e.__class__.__name__.replace("Error", "")
                print(LOG_RED + f"{proxy_name} | {symbol} | {api_name} | {error_msg} | Retrying...")
...
async def fetch_and_process_symbol(session, base_symbol, semaphore, db_manager, state, proxy_map, is_final_loop):
    async with semaphore:
        ...
        results = await asyncio.gather(
            fetch_data(session, binance_symbol, BINANCE_API_CONFIG["ohlcv"], state, start_time_ms, proxy_map, is_final_loop, "OHLCV"),
            fetch_data(session, binance_symbol, BINANCE_API_CONFIG["pfr"], state, start_time_ms, proxy_map, is_final_loop, "PFR"),
        ...
        # --- BUG FIX: Insert OHLCV and PFR data in separate loops ---
        if processed_ohlcv:
...
async def main():
    ...
    print(LOG_CYAN + f"--- 🚀 Starting {SCRIPT_DEF} ---")
    print(LOG_CYAN + f"{len(BASE_SYMBOLS)} symbols | {DB_RETENTION_DAYS} days history | {len(CONFIGURED_PROXIES)} proxies active")
    
    async def heartbeat():
        while True:
            await asyncio.sleep(HEARTBEAT_INTERVAL_SECONDS)
            fetched_stats = "; ".join([f"{name}: {count:,}" for name, count in sorted(state.records_fetched.items())])
            print(LOG_CYAN + f"{SCRIPT_DEF} | Fetched: {fetched_stats} | Inserted: ~{state.total_records_inserted:,}")
    ...
    try:
        async with AsyncSession() as session:
            ...
            print(LOG_CYAN + "Historical Backfill Complete - Starting Final Loop (to Now)")
            
            await run_fetch_cycle(session, db_manager, state, proxy_map, is_final_loop=True)
            print(LOG_CYAN + "-- Final Loop Complete, MT token complete --")
            
    except Exception:
        print(LOG_RED + f"--- 💥 Unhandled Exception: Aborting Script ---")
        print(LOG_RED + traceback.format_exc())
    
    finally:
        heartbeat_task.cancel()
        duration = time.time() - script_start_time
        
        print(LOG_BOLD_WHITE + f"--{SCRIPT_DEF} Finished - Total Records Inserted: {state.total_records_inserted:,}")
        print(LOG_BOLD_WHITE + f"🕒 Script complete. Duration: {duration:.2f} seconds.")
