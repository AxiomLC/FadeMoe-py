...
class DBManager:
    def __init__(self):
        ...
        try:
            ...
            # Keep self.conn for backward compatibility with setup/query methods
...
#=======================================================================
    def insert_batch_data(self, data):
        """
        Inserts a batch of data with connection pooling and retry logic.
        ...
        if not self.pool:
            print("❌ No database connection pool.")
            return 0
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    print(f"⚠️ Deadlock detected, retrying in {wait_time:.2f}s...")
                    time.sleep(wait_time)
                else:
                    print(f"❌ Batch insert failed after {max_retries} retries: deadlock")
                    return 0
                ...
                if conn:
                    self.pool.putconn(conn)  # Always return connection to pool
                    conn = None
